 ...
 *
 * This tool extracts specific sections from HeroUI component documentation,
 * enabling users to retrieve targeted content like usage examples, API tables,
 * or specific component variants instead of the entire documentation.
 */
...
export class GetComponentUsageTool extends BaseTool {
	readonly name = "get_component_usage";
	readonly title = "Get Component Usage Tool";
	readonly description =
		"Extract usage examples and key sections from HeroUI component documentation";
	...
	async execute(
		...
		// Define the sections to extract automatically (most commonly needed)
		const sectionsToExtract = ["Usage"];
		...
		try {
			...
			// Extract the predefined sections
			...
			// If no standard sections found, try to get at least the Usage section
			if (extractedSections.length === 0) {
				const usageContent = extractSectionWithSubsections(
					mdxContent,
					"usage",
					true,
				...
				if (usageContent) {
					extractedSections.push(usageContent);
				} else {
					return {
						content: [
							{
								type: "text",
								text: `No usage or key sections found for component '${componentName}'. Available sections: ${this.getAvailableSections(mdxContent).join(", ")}`,
							},
						],
					};
				}
			...
			logger.debug("Successfully extracted component usage sections", {
				componentName,
			...
			logger.error("Error extracting component usage:", error);
			return {
				content: [
					{
						type: "text",
						text: `Error extracting usage from component documentation for '${componentName}': ${error instanceof Error ? error.message : "Unknown error"}`,
					},
				],
			};
		}
